...
educ_levels <- c("ed_all", "ed_prim", "ed_sec_low", 
                 "ed_sec_high", "ed_sup") |> 
...
Códigos INE de los municipios, provincias y regiones con sus nombres.

```{r}
reg_codes <- read_csv(data_file("ine_reg_codes.csv"), 
                      col_types = "c")

prov_codes <- read_csv(data_file("ine_prov_codes.csv"), 
                      col_types = "_cc")

mun_codes <- read_csv(data_file("ine_mun_codes.csv"),
                      col_types = "cccc") |>
  mutate(mun_code = glue("{prov_code}{mun_order}")) |> 
  left_join(reg_codes, by = join_by(reg_code)) |> 
  left_join(prov_codes, by = join_by(prov_code)) |> 
  relocate(mun_code)
```

En el Atlas siguen apareciendo municipios que ya no existen, que han cambiado de provincia o que se han segregado de otros. 
...
drop_mun <- 
  tribble(
    ...
    "48036", "Galdako",      "Usansolo se segrega en el año 2023",
    "48916", "Usansolo",     "Segregado de Galdakoa oficialmente en diciembre de 2023",
  ) 

drop_mun |> tt()
```

Consolidación de los conjuntos de datos del INE. En el resultado final se excluyen también los municipios con menos de 500 habitantes, para los cuales no hay información sobre el nivel de estudios completado.

```{r}
ine_data <- mun_codes |> 
  filter_out(mun_code %in% pull(drop_mun, Código)) |>
  left_join(filter(pop_data), by = join_by(mun_code)) |> 
  left_join(age_data, by = join_by(mun_code, year)) |> 
  right_join(educ_data, by = join_by(mun_code, year)) |> 
  left_join(select(atlas_data, 
                   -c(pop, avg_age, pop_lt_18_pct, 
                      pop_gt_65_pct, pop_nat_pct)), 
            by = join_by(mun_code, year))

save_data(ine_data, "ine_data.rds")
```
