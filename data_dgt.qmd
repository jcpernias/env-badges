...
read_dgt_file <- function(input_file, year, dict) {
  ...
  read_xlsx(input_file, skip = 1,
            ...
            col_names = dict$name) |>
    filter_out(str_detect(mun, 
                          "[Mm]unicipio [Ss]in [Ee]specificar")) |> #<2>
    mutate(year = year) |> #<3>
    select(-mun) |> 
    relocate(year, .after = ine_code) |> 
    relocate(badge_none, .before = badge_b)
}
```
...
Se leen los datos de cada año y se combinan en un único conjunto de datos:
...
db <- 
  ...
  mutate(year = factor(year), #<1>
         badge_all = rowSums(across(badge_none:badge_0))) #<2>
```

1.  Convierte `year` en un factor.
2.  Calcula el total de vehículos con distintivo (no coincide con `fleet`).
...
frml <- (badge_none + badge_b + badge_c + 
           badge_eco + badge_0 + badge_all) * (Year = year) ~
  N + (Mean + SD + Median)

db |> 
  datasummary(frml, data = _) |> 
  tt_style(j = 2:4, align = "r")
```
